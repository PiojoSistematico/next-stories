...
const TopStories: React.FunctionComponent<TopStoriesProps> = ({
  ...
  return (
    <section className="bg-blue-200 flex flex-col gap-4 rounded-md overflow-hidden">
      <h2 className="bg-blue-900 text-blue-200 px-4 py-2 font-bold">{title}</h2>
      <ul className="flex flex-col gap-2 text-blue-950 p-2">
        {stories.map((story) => (
          <li className="p-4 bg-blue-300 shadow-sm shadow-blue-700 flex flex-row justify-between">
            <div className="flex flex-col gap-4">
              <p className="font-medium">{story.title}</p>
              <div className="flex flex-row gap-2">
                {story.tags.map((tag) => (
                  <span className="bg-blue-700 text-blue-100 rounded-sm px-2 py-1 text-sm">
                    {tag}
                  </span>
                ))}
              </div>
